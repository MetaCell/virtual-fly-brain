...
function getResultsSOLR(searchString, returnResults, sorter, configuration) {
  var url = configuration && configuration.url ? configuration.url : globalConfiguration.url;

  if (configuration && configuration.query_settings !== undefined) {
...
function refineResults(e) {
  const sorter = function (a, b) {
      ...
      // match ignoring joining non-words
      ...
      // pick up any match without non-word join character match
      ...
      // if the match is closer to the start, then move up
      ...
      // if the match in the id is closer to the start, then move up
...
const searchConfiguration = {
  ...
  "sorter": function (a, b) {
      ...
      if (InputString.toLowerCase().indexOf(' ') > -1) {
          var lcInputStringFac = InputString.toLowerCase().split(' ');
          var compare = (a1, a2) => a1.filter(v => a2.includes(v)).length;
          var cA = compare(lcInputStringFac, a.label.toLowerCase().split(' '));
          var cB = compare(lcInputStringFac, b.label.toLowerCase().split(' '));
          if (cA > 0 || cB > 0) {
              if (cA > cB) {
                  return -1;
              }
      ...
      // find all tokenized word matches
      if (InputString.split(/\W+/).length > 1) {
          var lcInputStringFac = InputString.toLowerCase().split(/\W+/);
          var compare = (a1, a2) => a1.filter(v => a2.includes(v)).length;
          var cA = compare(lcInputStringFac, a.label.toLowerCase().split(/\W+/));
          var cB = compare(lcInputStringFac, b.label.toLowerCase().split(/\W+/));
          if (cA > 0 || cB > 0) {
              if (cA > cB) {
                  return -1;
              }
      ...
      // prioritize matches in the primary label
      if (InputString.split(/\W+/).length > 1) {
          var lcInputStringFac = InputString.toLowerCase().split(/\W+/);
          var compare = (a1, a2) => a1.filter(v => a2.includes(v)).length;
          ...
          bLabel = bLabel.join(' (');
          var cA = compare(lcInputStringFac, aLabel.toLowerCase().split(/\W+/));
          var cB = compare(lcInputStringFac, bLabel.toLowerCase().split(/\W+/));
          if (cA > 0 || cB > 0) {
              if (cA > cB) {
                  return -1;
              }
