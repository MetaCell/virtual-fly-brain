 ...
 */
export const ControlsMenu = (component) => {
  const path = component.value._root.nodes.find((e) => e.entry?.[0] == "path")
    .entry[1];
...
export const NameComponent = (component) => {
  const entityName = component.value._root.nodes.find(
    (e) => e.entry?.[0] == "name",
  ).entry[1];
  const entityPath = component.value._root.nodes.find(
    (e) => e.entry?.[0] == "path",
  ).entry[1];
  const entitySelected = component.value._root.nodes.find(
    (e) => e.entry?.[0] == "selected",
  ).entry[1];

  return (
    <div style={{ width: "100%", textAlign: "left", float: "left" }}>
      <Link
        component="button"
        underline="none"
        variant="subtitle1"
...
export const TypeComponent = (component) => {
  let entityType = null;
  let entityPath = null;
  component.value._root.nodes.forEach((e) =>
    e.nodes?.forEach((n) => {
      if (n.entry?.[0] == "types") {
        entityType = n.entry?.[1];
      }
    }),
  );
...
export const TagsComponent = (component) => {
  let tags = null;
  component.value._root.nodes.forEach((e) =>
    e.nodes?.forEach((n) => {
      if (n.entry?.[0] == "tags") {
        ...
        return (
          <Chip
            ...
            style={{
              ...
              backgroundColor:
                facets_annotations_colors[tag]?.color ||
                facets_annotations_colors?.default?.color,
              color: "#ffffff",
            }}
