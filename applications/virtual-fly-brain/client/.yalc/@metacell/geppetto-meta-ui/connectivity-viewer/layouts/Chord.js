...
var d3 = require("d3");
var Chord = exports.Chord = /*#__PURE__*/function () {
  function Chord(calculateNodeDegrees) {
    _classCallCheck(this, Chord);
    this.calculateNodeDegrees = calculateNodeDegrees;
  }
  return _createClass(Chord, [{
    key: "draw",
    value: function draw(context) {
      ...
      var halfpi = pi / 2;
      var dr = 15;
      var innerRadius = Math.min(context.width, context.height) * .41,
      ...
      var chord = this.layout_chord().padding(.05).matrix(matrix);
      chord.groups().forEach(function (g, i) {
        g.id = context.dataset.nodeTypes[i];
      });
      var slice = svg.append("g").selectAll("path").data(chord.groups).enter().append("path").style("fill", function (d) {
        return fill(d.id);
      }).style("stroke", function (d) {
        return fill(d.id);
      }).attr("d", d3.arc().innerRadius(innerRadius).outerRadius(outerRadius)).on("mouseover", fade_over(.1)).on("mouseout", fade_out(1));
      slice.append("title").text(function (d) {
        return d.id;
      });
      var svg_asymChord = function svg_asymChord() {
        ...
        function chord(d, i) {
          ...
          var t_p0s = shifted[0];
          var t_p1s = shifted[1];
          return "M" + s.p0 + arc(s.r, s.p1, s.a1 - s.a0) + (equals(s, t) ? curve(s.r, s.p1, s.r, s.p0) : curve(s.r, s.p1, t.r, t_p0s) + arc(t.r - dr, t_p1s, t.a1 - t.a0) + curve(t.r - dr, t_p1s, s.r, s.p0)) + "Z";
        }
        function shift_radius(d) {
          return [[d.p0[0] - dr * Math.cos(d.a0), d.p0[1] - dr * Math.sin(d.a0)], [d.p1[0] - dr * Math.cos(d.a1), d.p1[1] - dr * Math.sin(d.a1)]];
        }
      ...
      svg.append("g").attr("class", "chord").selectAll("path").data(chord.chords).enter().append("path").attr("d", function (d, i) {
      ...
      function fade_over(opacity) {
        return function (g, i) {
          ...
          svg.selectAll(".chord path").filter(filter).transition().style("opacity", opacity);
        };
      }
    ...
    key: "generateChordMatrix",
    value: function generateChordMatrix(context) {
      var matrix = [];
      var type2type = {};

      // {type_i: {postType_j: counts, ...}, ...}
      var typesZeros = util.map(context.dataset.nodeTypes, function (type) {
        return [type, 0];
      });
      context.dataset.nodeTypes.forEach(function (type) {
        var initCounts = util._object(typesZeros);
        var linksFromType = util.filter(context.dataset.links, function (link) {
    ...
    value: function layout_chord() {
      ...
      function relayout() {
        ...
        while (++i < n) {
          j = -1;
          while (++j < n) {
            var source = subgroups[i + "-" + j + "_pre"],
              target = subgroups[i + "-" + j + "_post"];
